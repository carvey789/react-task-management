...
import { SensorContext } from "../types/types";
    ...
    if (directions.includes(event.code)) {
      ...
      if (closestId && over?.id) {
        ...
        if (activeRect && newRect && newDroppable) {
          const newIndex = cards.findIndex(({ id }) => id === closestId);
          ...
          if (newItem && activeItem) {
            const { depth } = getProjection(
              ...
              active.id,
              closestId,
              (newItem.cardDepth - activeItem.cardDepth) * indentationWidth,
              indentationWidth
            );
